...
class DependencyBertFusion(nn.Module):
...
class DependencyBertFusionAttention(nn.Module):
    def __init__(self, config):
        super().__init__()
        self.self = DependencyBertFusion(config)
...
class DependencyBertFusionLayer(GradientCheckpointingLayer):
    def __init__(self, config):
        super().__init__()
        self.chunk_size_ff = config.chunk_size_feed_forward
        self.seq_len_dim = 1
        self.attention = DependencyBertFusionAttention(config)
...
class DependencyBertModel(BertPreTrainedModel):
    def __init__(self, config):
        ...
        self.fusion = DependencyBertFusionLayer(config)
