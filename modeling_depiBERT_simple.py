...
class SimpleDependencyBertMix(nn.Module):
    ...
    def mix(self, self_attn, dep_self_attn):
        output = (self_attn + dep_self_attn)/2  # [B, nh, T, T]
        return output

class SimpleDependencyBertMixAttention(nn.Module):
    def __init__(self, config):
        super().__init__()
        self.self = SimpleDependencyBertMix(config)
...
class SimpleDependencyBertMixLayer(GradientCheckpointingLayer):
    def __init__(self, config):
        super().__init__()
        self.chunk_size_ff = config.chunk_size_feed_forward
        self.seq_len_dim = 1
        self.attention = SimpleDependencyBertMixAttention(config)
...
class SimpleDependencyBertModel(BertPreTrainedModel):
    def __init__(self, config):
        super().__init__(config)
        self.config = config

        self.embeddings = BertEmbeddings(config)
        self.mixer = SimpleDependencyBertMixLayer(config)
...
custom_intro="""
    ...
    """
class SimpleDependencyBertForPretraining(BertPreTrainedModel):
    _tied_weights_keys = ["predictions.decoder.bias", "cls.predictions.decoder.weight"]

    def __init__(self, config):
        super().__init__(config)

        self.depBert = SimpleDependencyBertModel(config)
...
class SimpleDependencyBertForSequenceClassification(BertPreTrainedModel):
    def __init__(self, config):
        super().__init__(config)
        self.config = config
