...
class MultiDependencyBertSelfAttention(nn.Module):
    ...
    def forward(self, 
        ...
        self_attn_probs = F.softmax(mixed_self_attn, dim=-1)    # [B, nh, T, T]
        self_attn_probs = self.attention_probs_dropout_prob(self_attn_probs)    # [B, nh, T, T]
        context_layer = torch.matmul(self_attn_probs, v)    # [B, nh, T, nh]
...
class MultiDependencyBertAttention(nn.Module):
    def __init__(self, config):
        super().__init__()
        self.self = MultiDependencyBertSelfAttention(config)
...
class MultiDependencyBertLayer(nn.Module):
    def __init__(self, config):
        super().__init__()
        self.chunk_size_ff = config.chunk_size_feed_forward
        self.seq_len_dim = 1
        self.attention = MultiDependencyBertAttention(config)
...
class MultiDependencyBertEncoder(nn.Module):
    def __init__(self, config):
        super().__init__()
        self.layer = nn.ModuleList([MultiDependencyBertLayer(config) for _ in range(config.num_hidden_layers)])
    ...
    def forward(
        ...
        for layer_module in self.layer:
            ...
            if output_attentions:
                all_attn = all_attn + (layer_outputs[1],)
...
class MultiDependencyBertModel(BertPreTrainedModel):
    def __init__(self, config):
        super().__init__(config)
        self.config = config

        self.embeddings = BertEmbeddings(config)
        self.encoder = MultiDependencyBertEncoder(config)
...
custom_intro="""
    ...
    """
class MultiDependencyBertForPretraining(BertPreTrainedModel):
    _tied_weights_keys = ["predictions.decoder.bias", "cls.predictions.decoder.weight"]

    def __init__(self, config):
        super().__init__(config)

        self.depBert = MultiDependencyBertModel(config)
...
class MultiDependencyBertForSequenceClassification(BertPreTrainedModel):
    def __init__(self, config):
        super().__init__(config)
        self.config = config
